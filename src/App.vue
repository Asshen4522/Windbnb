...
function onLaunch() {
  const listLoc = new Set();
  information.cards.forEach((element) => {
    let stroka = element.city + "." + element.country;
    listLoc.add(stroka);
  });
  listLoc.forEach((element) => {
    information.locationVar.push(element);
  });
}
